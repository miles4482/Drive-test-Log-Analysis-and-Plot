...
def apply_xy_levels(
    ...
    horizontal tick labels or a usable vertical scale â€” the bug in the v1.3 charts.
    """
    ...
    if y_major_unit is not None:
        try:
            ...
            pass
    if rotate_x:
        # Rotation only â€” do not add an empty text run, which blanks tick labels.
...
def build_cover(ws: Worksheet, df: pd.DataFrame) -> None:
    ...
    info_rows = [
        ...
        ["Longitude range", f"{df['Longitude'].min():.5f} â€“ {df['Longitude'].max():.5f}"],
        ["Latitude range", f"{df['Latitude'].min():.5f} â€“ {df['Latitude'].max():.5f}"],
    ]
...
def build_kpi_sheet(ws, df, name, color, unit, map_path: Path | None, placeholder_path: Path | None = None):
    ...
    set_widths(ws, {get_column_letter(i): 14 for i in range(1, last_col + 1)})
    section_bar(ws, 4, f"{name} coverage map â€” IDLE Mode", 1, 8)
    section_bar(ws, 4, f"{name} coverage map â€” Active Mode", 10, 18)
...
def save_report_preview(df: pd.DataFrame, path: Path) -> Path:
    ...
    fig.suptitle("Bogura Drive-Test Report â€” RSRP / RSRQ / SINR", fontsize=13, fontweight="bold")
    fig.tight_layout()
    fig.savefig(path, dpi=140, facecolor="white")
    plt.close(fig)
    return path
